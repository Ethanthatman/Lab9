...
void insertRecord(struct HashType *hashTable, struct RecordType *record, int tableSize)
{
    ...
    {
        struct RecordType* tp = hashTable[index].table;
        while(tp->next != NULL)
        {
            tp = tp->next;
        }
...
void displayRecordsInHash(struct HashType *hashTable, int tableSize)
{
    ...
    {
        printf("Index %d -> ",x);
        struct RecordType* temp = hashTable[x].table; 
        while(temp != NULL)
        {
...
int main(void)
{
    ...
    struct HashType *table = calloc(hashTableSize,sizeof(struct HashType*));

    struct RecordType *temp = pRecords;
    for(int x = 0; x < recordSz; x++)
    {
        insertRecord(table, pRecords + x, hashTableSize);
    }

    displayRecordsInHash(table, hashTableSize);
